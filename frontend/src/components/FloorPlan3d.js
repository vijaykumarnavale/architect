      ...
      <div className="grid grid-cols-2 gap-4 mb-4 p-6 border rounded-lg shadow-lg bg-white">
        ...
        <div className="col-span-2 flex justify-center mt-4">
          <button
            ...
            disabled={loading}
            className={`px-6 py-3 rounded-lg shadow-md text-white transition-all ${
      ...
      <div className="overflow-x-auto w-full flex flex-wrap gap-6 mt-6">
        {floorPlans.length > 0 ? (
          floorPlans.map((plan, index) => (
            <div key={index} className="w-full h-[500px] border rounded-lg shadow-lg p-4 bg-white">
              ...
              <Canvas shadows camera={{ position: [10, 10, 20], fov: 50 }}>
                ...
                <Suspense fallback={<Html><p className="text-gray-500">Loading 3D Plan...</p></Html>}>
                  <Room plan={plan} width={formData.width} depth={formData.depth} />
                </Suspense>
                <OrbitControls />
              </Canvas>
            </div>
...
const Room = memo(({ plan, width, depth }) => {
  ...
  return (
    <>
      ...
      {plan.map((line, index) => (
        <Wall key={index} start={line.start} end={line.end} color="#cfcfcf" xOffset={xOffset} zOffset={zOffset} />
      ))}
    </>
  );
});
